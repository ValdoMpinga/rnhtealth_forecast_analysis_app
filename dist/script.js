...
class ForecastAnalysis {
    ...
    addToForecastData(forecastHour, data) {
    ...
    calculateForecastMetrics(forecastHour) {
        const forecastKey = `${forecastHour}HourForecastData`;
        ...
        data.forEach((item) => {
            ...
            const bilstmAbsolutePercentageError = (bilstmError / item.measurement) * 100;
        ...
        const metrics = {
            lstm: {
                mae: lstmErrors.reduce((sum, error) => sum + error, 0) / lstmErrors.length,
                rmse: Math.sqrt(lstmErrors.reduce((sum, error) => sum + Math.pow(error, 2), 0) /
                    lstmErrors.length),
                mape: lstmAbsolutePercentageErrors.reduce((sum, error) => sum + error, 0) /
                    lstmAbsolutePercentageErrors.length,
            },
            bilstm: {
                mae: bilstmErrors.reduce((sum, error) => sum + error, 0) /
                    bilstmErrors.length,
                rmse: Math.sqrt(bilstmErrors.reduce((sum, error) => sum + Math.pow(error, 2), 0) /
                    bilstmErrors.length),
                mape: bilstmAbsolutePercentageErrors.reduce((sum, error) => sum + error, 0) / bilstmAbsolutePercentageErrors.length,
            },
        };
